...
date-format: long
abstract: "In this paper, we study reported shootings and gun incidents in Toronto from 2018 to 2024. Our analysis identifies key trends over time and in different neighborhoods, showing areas of higher risk, seasons, and times of day. The data indicates a possible relationship between the number of incidents and different areas of Toronto, as well as the time of day. This study provides important insights that will help the Toronto Police Service improve security measures and effectively target high-risk locations with increased police presence."
format: pdf
toc: true
toc-depth: 3
...
# Introduction
We used @citeR to analyze the data and plot the graphs for this paper. The folder structure for this paper follows the startup folder created by @rohan. It is very helpful in keeping everything organized. I also referenced many of the examples and techniques provided by @alexander2023telling, which helped me understand how to visualize the data and communicate the findings effectively. We used @citedplyr to process and transform the dataset for this paper efficiently, which made it easier to filter, group, and summarize the data. In addition, the @citetidyverse packages is essential for simplifying data cleaning and analysis, providing a consistent framework for working with data in R. These tools help to organize and present data efficiently.

Shootings and gun discharges have become a growing concern in Toronto. The increase in gun-related incidents not only threatens public safety, but also stresses policymakers, law enforcement and local communities. Understanding the causes of these events, as well as high-risk areas, is essential to developing strategies to fight gun violence. Detailed analysis of data related to shooting incidents can reveal important insights to help identify high-risk areas and times and provide solutions to reduce violence.

Despite the growing concern about gun violence, the public remains poorly informed about how specific factors such as time and place influence gun violence. This requires a more comprehensive analysis of the data. In this paper, I analyze Toronto's reported shooting and gun discharge incidents between 2018 and 2024 and provide the public with relevant information in a brief, easy-to-read chart. We utilize “year” and “month” variables to reveal trends in the number of victims over time and utilize latitude and longitude data to map the density of victims in order to determine which regions and time periods are at a higher risk of having a shooting incident. In addition, we explore the relationship between the hours of the day and the number of related cases.

Data on shooting incidents over the past two decades show fluctuations in the number of victims from year to year, but the number of the incidence has increased in recent years. The frequency of shootings is significantly higher during the summer months, and the incidents tend to occur at night, especially between 9:00 p.m. and 2:00 a.m. The data shows that the most dangerous areas are centered in the downtown area. The data show that the most dangerous areas are concentrated in the city centers. These findings highlight specific times and places that may require increased attention in terms of crime prevention and security measures.

The rest of the paper is structured as follows: in @sec-data, we describe the source of the dataset and its context in detail. Next, we show our findings through various types of graphs with visualizations. After each chart, we discuss each graph's formation, the significance of the findings, and finally summarize key observations. Finally, there is an appendix and references. By the end of this paper, the reader will have a clearer understanding of trends in gun violence in Toronto and how this data can be utilized to improve safety measures throughout the city.
...
ggplot(yearly_data, aes(x = year, y = total_people)) + 
...
ggplot(cleaned_data, aes(x = month, y = total_people, fill = month)) +
  ...
  theme_minimal()
```
@fig-deaths-injuries-month shows the total number of shootings and gun discharges by month. The chart uses the “month” variable to show the distribution of incidents throughout the year. From the graph we can observe that the number of shooting victims peaks in the summer months, especially in July and August. This shows that shootings tend to occur more frequently in the warmer months, which may be due to an increase in outdoor activities. Winter months have significantly fewer incidents, suggesting that there may be a seasonal pattern to gun violence in Toronto.
...
ggplot(map_data, aes(x = Longitude, y = Latitude, color = Risk_Level)) +
  ...
  labs(title = "Map of Incidents Highlighted by Risk Level",
...
The first step involves loading the original dataset, normalizing and simplifying the column names. This makes the data easier to analyze. Only the most important variables were kept, focusing on information such as the time, location and outcome (e.g., deaths and injuries) of the event.
...
After cleaning and transforming the data, the final version was saved in a new file.
